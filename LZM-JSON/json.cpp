...
string Json::str() const
{
	...
	switch (m_type)
	{
	case lzm::json::Json::json_null:
		ss << "null";
		break;
	case lzm::json::Json::json_bool:
		if (m_value.m_bool)
		{
			ss << "true";
		}
		...
		break;
	case lzm::json::Json::json_int:
		ss << m_value.m_int;
		break;
	case lzm::json::Json::json_double:
		ss << m_value.m_double;
		break;
	case lzm::json::Json::json_string:
		ss << '\"' << *(m_value.m_string) << '\"';
		break;
	case lzm::json::Json::json_array:
		{
		ss << '[';
		for (auto it = (m_value.m_array)->begin(); it != (m_value.m_array)->end(); it++)
		{
	...
	case lzm::json::Json::json_object:
	{
		ss << '{';
		for (auto it = (m_value.m_object)->begin(); it != (m_value.m_object)->end(); it++)
		{
